...
async function main() {
  ...
  // Создаем индивидуальный маркер для каждой связи
  const markers = defs.selectAll("marker")
    .data(links)
    .join("marker")
    .attr("id", d => `arrow-${d.source.id || d.source}-${d.target.id || d.target}`)
  ...
  // Функция для обновления refX (вызываем в tick)
  function updateMarkers() {
    defs.selectAll("marker")
      .attr("refX", d => {
        // Индивидуальный расчет: половина ширины целевого узла + длина стрелки (10)
        const targetWidth = d.target.bboxWidth || 100;
        return (targetWidth / 2) + 10;
      });
  }
  // 6. Отрисовка узлов
  ...
  const link = g.append("g")
    ...
    .attr("stroke", "#999")
    .attr("fill", "none")
    .attr("marker-end", d => `url(#arrow-${d.source.id || d.source}-${d.target.id || d.target})`);
  ...
  simulation.on("tick", () => {
    // Линии рисуем строго В ЦЕНТР (как вы просили)
    link.attr("d", d3.linkHorizontal()
      .x(d => d.x)
      .y(d => d.y)
    );
    // Обновляем индивидуальные смещения стрелок
    updateMarkers();
